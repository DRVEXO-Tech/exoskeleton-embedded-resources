...
void DRVEXOBiasInitialise(DRVEXOBias *const bias, const unsigned int sampleRate) {
    ...
    bias->timer = 0;
    bias->gyroscopeOffset = DRVEXO_VECTOR_ZERO;
}
...
DRVEXOVector DRVEXOBiasUpdate(DRVEXOBias *const bias, DRVEXOVector gyroscope) {
    ...
    gyroscope = DRVEXOVectorSubtract(gyroscope, bias->gyroscopeOffset);
    ...
    bias->gyroscopeOffset = DRVEXOVectorAdd(bias->gyroscopeOffset, DRVEXOVectorScale(gyroscope, bias->filterCoefficient));
    return gyroscope;
}
