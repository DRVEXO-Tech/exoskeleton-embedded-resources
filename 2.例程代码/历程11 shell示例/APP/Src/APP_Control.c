...
/**
 * @brief 发送模式切换灯光命令到左右电机。
 * @param mode BLE 下发的模式值。
 * @return 返回发送结果。
 */
int32_t APP_Control_SetBleSystemMode(uint32_t mode, uint32_t Timeout)
{
    int32_t ret;
...
/**
 * @brief 发送力矩档位灯光切换命令到左右电机。
 * @param torque BLE 下发的力矩档位值。
 * @return 返回发送结果。
 */
int32_t APP_Control_SetBleSystemTorque(uint8_t torque, uint32_t Timeout)
{
    int32_t ret;

    memset(control_buf, 0, sizeof(control_buf));
    control_buf[0] = 34;
    control_buf[1] = torque;
    ret = app_control_send_command_timeout(Left, 2, Timeout);
    if (ret != LY_OK)
    {
        return ret;
